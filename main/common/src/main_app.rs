...
impl MainApp {
    ...
    pub fn run(self) {
        ...
        let (msg_sender, msg_receiver) = interprocess_channel::bounded(cap);

        let msg_receiver = MpscReceiver::bounded(msg_receiver, cap);

        let msg_sender_handle: OwnedHandle = msg_sender.into();
        let msg_sender_raw_handle = msg_sender_handle.as_raw_handle() as usize;

        let path = crate::get_current_dir().join("global_listener_app");

        let global_listener = std::process::Command::new(path)
...
struct App {
    global_listener: std::process::Child,
    global_listener_channel_handle: ManuallyDrop<OwnedHandle>,
...
impl App {
    pub fn new(
        ...
        msg_receiver: MpscReceiver<InstantWrap>,
        global_listener: std::process::Child,
        global_listener_channel_handle: OwnedHandle,
    ) -> Self {
        ...
        Self {
            global_listener,
            global_listener_channel_handle: ManuallyDrop::new(global_listener_channel_handle),
            msg_receiver,
...
impl Drop for App {
    fn drop(&mut self) {
        let hwnd = unsafe {
            FindWindowW(
                global_listener_app::MainApp::window_class_name(),
                global_listener_app::MainApp::window_name(),
            )
        }
        .unwrap();
        unsafe { PostMessageW(hwnd, WM_CLOSE, None, None) }.unwrap();
        self.global_listener.wait().unwrap();

        unsafe { ManuallyDrop::drop(&mut self.global_listener_channel_handle) };
    }
}
